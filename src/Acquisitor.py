        ...
        if(mms_analysis):
            ...
            for i in range (0, Ntimes):
                if True:
                    ...
                    #     ax2.scatter(ini[k, 0], ini[k, 1], ini[k,2], s=40, lw=1, edgecolors="black")
                    ...
                    # fig.savefig(f'3d_plot_time{i}.png',dpi=150)
                    # plt.close()
            ...
            # plt.show()
            # np.save('AIC_info',[info_plot,nclusters_plot],allow_pickle=True)



            plt.plot(all_Etherm)
            plt.ylabel('Etherm (from best #components)')
            plt.xlabel('time')
            plt.show()
            np.save('Etherm_1component',[all_Etherm],allow_pickle=True)
            # plt.clf()        
            ...
            # plt.title(self.information_criterion)
            # plt.xlabel('time')
            # plt.ylabel('n of clusters')
            # # plt.colorbar(cf,format='%.10f')
            # plt.colorbar()
            # plt.savefig('colorplot_'+self.information_criterion+'.png',dpi=300)
